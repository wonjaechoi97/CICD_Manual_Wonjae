...
- 참고 : jenkins bash에서 나가기 위해서는 `exit` 입력을 통해
...
# WORKDIR /var/jenkins_home/workspace/deploytest/디렉토리에 맞게
COPY package*.json ./
RUN npm install
COPY . .
RUN npm run build
...
# COPY --from=build-stage /var/jenkins_home/workspace/deploytest/frontend/yogidice/dist /usr/share/nginx/html <---vue
...
WORKDIR /var/jenkins_home/workspace/deploytest/yogidice
# ARG JAR_FILE=build/libs/*.jar
# COPY ${JAR_FILE} app.jar
COPY --from=build-stage build/libs/*.jar app.jar

EXPOSE 8080
ENTRYPOINT [ "java", "-jar", "/var/jenkins_home/workspace/deploytest/yogidice/app.jar" ]
```
...
- if (sudo docker ps | grep "react"); then sudo docker stop vue; fi : vue 컨테이너가 만약 동작중이면 stop 시키기
...
COPY --from=build-stage /var/jenkins_home/workspace/deploytest(젠킨스 프로젝트)/frontend/yogidice/dist /usr/share/nginx/html  # react의 경우 dist->build
COPY --from=build-stage /var/jenkins_home/workspace/eploytest(젠킨스 프로젝트)/frontend/yogidice/deploy_conf/nginx.conf /etc/nginx/conf.d/default.conf
EXPOSE 80
CMD ["nginx", "-g","daemon off;"]
```
